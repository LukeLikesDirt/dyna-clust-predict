#!/bin/bash
# Script name:  06b_launch_parallel.sh
...
#               Launches 06c_predict_cutoffs_parallel.sh once per region.
#
# Usage:        bash scripts/06b_launch_parallel.sh
# Note:         Must be run from the project root directory.

set -euo pipefail

SCRIPT="scripts/06c_predict_cutoffs_parallel.sh"
